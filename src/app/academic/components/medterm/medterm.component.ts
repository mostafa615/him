...
@Component({
  selector: 'app-medterm',
  templateUrl: './medterm.component.html',
  styleUrls: ['./medterm.component.scss']
})
export class MedtermComponent implements OnInit {
  filter: any = {};
  $: any = $;
...
load() {
  ...
  if (!Helper.validator(this.filter, ['year_id'])) {
    ...
    this.globalService.loadHtml("affair/report23", this.filter).subscribe((res) => {
